...
export default class ManagePosition extends React.Component {
  ...
  componentDidMount() {
    this.getListData();
    // 初始化字典
    Dictionary.getInstance().init("card_channel,UserTypeCD,Common_BusinessCode,pay_type,card_trade_type").then((value) => {
      this.setState({
        card_channel: value.card_channel,
        UserTypeCD: value.UserTypeCD,
        Common_BusinessCode: value.Common_BusinessCode,
        pay_type: value.pay_type,
        card_trade_type: value.card_trade_type
      });
        console.log("Common_BusinessCode");
        console.log(this.state.card_channel);
    })
  }
